...
export default function AdminTipsManager() {
  ...
  const [extIdsText, setExtIdsText] = useState(""); // externalIds separados por coma/espacios/nueva línea
  const [tagsText, setTagsText] = useState("");     // tags separados por coma/espacios/nueva línea
  ...
  const externalIds = useMemo(() => splitCsv(extIdsText).filter(Boolean), [extIdsText]);
  const tags = useMemo(() => splitCsv(tagsText).map(t => t.toLowerCase()).filter(Boolean), [tagsText]);
