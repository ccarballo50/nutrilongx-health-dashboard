...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { data: ins, error: insErr } = await sb
      .from('action_logs').insert(insertRow).select('id, created_at').maybeSingle();
    if (insErr) return res.status(400).json({ error: 'Insert in action_logs failed', details: insErr, row: insertRow });
